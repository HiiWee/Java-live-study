package operatingsystem.page_replacement.mru;
...
class PageTable {
    ...
    private void faultPage(final Page page) {
        faultCount++;
        int minReferenceCount = getMinReferenceCount();
        Page findPage = findPage(minReferenceCount);
        replacePage(findPage, page);
        increaseCountOfExistPage(page);
    }

    private Page findPage(final int minReferenceCount) {
        return pages.stream()
                .filter(page -> page.getReferenceCount() == minReferenceCount)
                .findFirst()
                .get();
    }

    private int getMinReferenceCount() {
        return pages.stream()
